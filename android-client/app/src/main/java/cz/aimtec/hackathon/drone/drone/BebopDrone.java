...
public class BebopDrone {
    ...
    public void askForAllStates() {
        mDeviceController.getFeatureCommon().sendCommonAllStates();
    }
    ...
    private void notifyPositionChanged(double latitude, double longitude, double altitude) {
        List<IBebopListener> listenersCpy = new ArrayList<>(mListeners);
        for (IBebopListener listener : listenersCpy) {
            listener.speedChanged(latitude, longitude, altitude);
        }
    }

    private void notifySpeedChanged(double latitude, double longitude, double altitude) {
        List<IBebopListener> listenersCpy = new ArrayList<>(mListeners);
        for (IBebopListener listener : listenersCpy) {
            listener.positionChanged(latitude, longitude, altitude);
        }
    }

    private void notifyAttitudeChanged(double roll, double pitch, double yaw) {
        List<IBebopListener> listenersCpy = new ArrayList<>(mListeners);
        for (IBebopListener listener : listenersCpy) {
            listener.attitudeChanged(roll, pitch, yaw);
        }
    }
    ...
    private void notifyAltitudeChanged(double altitude) {
        List<IBebopListener> listenersCpy = new ArrayList<>(mListeners);
        for (IBebopListener listener : listenersCpy) {
            listener.altitudeChanged(altitude);
        }
    }
